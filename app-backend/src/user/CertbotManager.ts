...
class CertbotManager {
    ...
    enableSsl(domainName: string) {
        ...
        return Promise.resolve()
            .then(function() {
                const webrootInCaptainContainer =
        ...
        return Promise.resolve()
            .then(function() {
                ...
                if (!fs.existsSync(rootPathDir)) {
                    Logger.d(
                        'Fresh install of Certbot. There is no registration directory'
                    )
                    return undefined
                }
                ...
                if (files.length === 0) {
                    Logger.d(
                        'Fresh install of Certbot. There is nothing in the registration directory'
                    )
                    return undefined
                }
                ...
                if (files.length !== 1) {
                    throw new Error(
                        'I do not know know what to do when there are multiple directories in ' +
                            rootPathDir
                    )
                }
                ...
                if (!fs.existsSync(regFilePath)) {
                    throw new Error(
                        'ACME Reg directory exists, but there is no file! ' +
                            regFilePath
                    )
                }
            ...
            .then(function(regrContent) {
                if (!regrContent) {
                    ...
                    throw new Error(
                        'Previously registered with a different address: ' +
                        contact
    ...
    init(myNodeId: string) {
        ...
        const domainSpecificRootDirectoryInHost =
            CaptainConstants.captainStaticFilesDir +
            CaptainConstants.nginxDomainSpecificHtmlDir
