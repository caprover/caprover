	...
	},
	{
		...
		filter: async (value: string) => {
			...
			try {
				...
				const data = await LoginApi.loginMachine(`http://${ipFromUser}:3000`, DEFAULT_PASSWORD);
	...
	},
	{
		...
		filter: async (value: string) => {
			try {
				...
				await SystemApi.enableHttps(`http://${customDomainFromUser}`, emailAddressFromUser);

				const data = await SystemApi.forceHttps(`https://${customDomainFromUser}`);
	...
	},
	{
		...
		filter: async (value: string) => {
			...
			try {
				...
				const changePassData = await LoginApi.changePass(machineUrl, confirmPasswordValueFromUser);

				if (changePassData) {
					const loginData = await LoginApi.login(machineUrl, confirmPasswordValueFromUser);
